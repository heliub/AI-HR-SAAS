...
获取简历各状态的统计数据，管理员可查看所有简历统计，HR只能查看自己负责的简历统计
- **性能优化**: 使用 GROUP BY 一次性查询所有状态统计，避免多次查询
- **查询参数与列表接口保持一致**: 支持相同的过滤条件

**查询参数**:
- `search` (string, optional): 搜索关键词（姓名、邮箱、职位）
- `status` (string, optional): 简历状态筛选（统计特定状态下的数据）
- `jobId` (UUID, optional): 职位ID筛选

**请求示例**:
```http
GET /api/v1/resumes/stats
GET /api/v1/resumes/stats?jobId=30000000-0000-0000-0000-000000000001
GET /api/v1/resumes/stats?search=工程师&status=pending
Authorization: Bearer YOUR_TOKEN
```
...
**字段说明**:
- `total` (int): 简历总数
- `pending` (int): 待处理状态的简历数量
- `reviewing` (int): 审核中状态的简历数量
- `interview` (int): 面试中状态的简历数量
- `offered` (int): 已发offer状态的简历数量
- `rejected` (int): 已拒绝状态的简历数量
...
**权限说明**:
- **管理员 (`admin`角色)**: 可查看所有租户的简历统计
- **普通HR (`hr`角色)**: 只能查看自己负责的简历统计
...
**POST** `/candidate-conversations/create_conversation`

根据简历ID和职位ID创建新的候选人会话

**请求体**:
```json
{
  "resume_id": "40000000-0000-0000-0000-000000000001",
  "job_id": "30000000-0000-0000-0000-000000000001"
}
```

**字段说明**:
- `resume_id` (UUID, required): 候选人简历ID
- `job_id` (UUID, required): 职位ID
...
**GET** `/candidate-conversations/get_conversations`

获取当前用户有权限访问的会话列表

**查询参数**:
- `status` (string, optional): 会话状态过滤 (opened, ongoing, interrupted, ended)
- `stage` (string, optional): 会话阶段过滤 (greeting, questioning, intention, matched)
- `resume_id` (UUID, optional): 简历ID过滤
- `job_id` (UUID, optional): 职位ID过滤
- `limit` (int, optional): 返回数量，默认20，范围1-100
- `offset` (int, optional): 偏移量，默认0，用于分页

**请求示例**:
```http
GET /api/v1/candidate-conversations/get_conversations?status=ongoing&limit=10&offset=0
Authorization: Bearer YOUR_TOKEN
```
  ...
  "data": {
    ...
    "list": [
      ...
      },
      {
        ...
        "tenantId": "10000000-0000-0000-0000-000000000001",
        "userId": "10000000-0000-0000-0000-000000000004",
        "resumeId": "40000000-0000-0000-0000-000000000002",
        "jobId": "30000000-0000-0000-0000-000000000002",
        "status": "opened",
        "stage": "greeting",
        "summary": null,
...
**GET** `/candidate-conversations/{conversation_id}`

获取指定会话的详细信息，包含基本信息和最新10条消息
    ...
    "latestMessages": [
      {
        ...
        "resumeId": "40000000-0000-0000-0000-000000000001",
        "sender": "ai",
        "message": "您好！我是AI招聘助手，看到您对我们的前端工程师职位感兴趣，能简单介绍一下您的工作经验吗？",
      ...
      },
      {
        ...
        "resumeId": "40000000-0000-0000-0000-000000000001",
        "sender": "candidate",
        "message": "我有5年前端开发经验，主要使用React和TypeScript，之前在字节跳动工作过。",
        "messageType": "text",
        "metadata": null,
        "createdAt": "2025-01-27T10:32:15Z",
        "updatedAt": "2025-01-27T10:32:15Z"
      }
    ],
    "messageCount": 15,
    "createdAt": "2025-01-27T10:30:00Z",
...
**GET** `/candidate-conversations/{conversation_id}/messages`
...
**查询参数**:
- `limit` (int, optional): 返回消息数量，默认100，最大500
- `offset` (int, optional): 偏移量，默认0，用于分页
...
**POST** `/candidate-conversations/{conversation_id}/messages`

候选人发送消息，系统自动执行对话流程并返回AI回复
...
```json
{
  "content": "请问这个职位的薪资范围是多少？"
}
```

**字段说明**:
- `content` (string, required): 消息内容，1-2000字符
...
```json
{
  ...
  "data": {
    "candidateMessage": {
      ...
      "resumeId": "40000000-0000-0000-0000-000000000001",
      "sender": "candidate",
      "message": "请问这个职位的薪资范围是多少？",
      "messageType": "text",
      "metadata": null,
      "createdAt": "2025-01-27T11:00:00Z",
      "updatedAt": "2025-01-27T11:00:00Z"
    },
    "aiMessage":  "该职位的薪资范围是25K-45K，具体会根据您的经验和能力进行调整。此外，我们还有年终奖金和股票期权等福利。",
    "action": "send_message",
...
**字段说明**:
- `candidateMessage`: 候选人发送的消息
- `aiMessage`: AI回复的消息（可能为空）
- `action`: 流程动作 (send_message-发送消息, suspend-转人工, none-无动作)
