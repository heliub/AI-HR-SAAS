...
VALUES 
  ('10000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', 'AI助手', 'ai@demo.com', '$argon2id$v=19$m=65536,t=2,p=2$mwLjoyW1IAMEgPeZk9tpYw$MvXStik3nxoAAvMR0NCa4KKIcEj8VGl9My6yD116clE', 'admin', TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('10000000-0000-0000-0000-000000000002', '00000000-0000-0000-0000-000000000001', '王总监', 'wang@demo.com', '$argon2id$v=19$m=65536,t=2,p=2$mwLjoyW1IAMEgPeZk9tpYw$MvXStik3nxoAAvMR0NCa4KKIcEj8VGl9My6yD116clE', 'hr', TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('10000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000001', '张经理', 'zhang@demo.com', '$argon2id$v=19$m=65536,t=2,p=2$mwLjoyW1IAMEgPeZk9tpYw$MvXStik3nxoAAvMR0NCa4KKIcEj8VGl9My6yD116clE', 'recruiter', TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('10000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000001', '李技术总监', 'li@demo.com', '$argon2id$v=19$m=65536,t=2,p=2$mwLjoyW1IAMEgPeZk9tpYw$MvXStik3nxoAAvMR0NCa4KKIcEj8VGl9My6yD116clE', 'admin', TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
ON CONFLICT (id) DO NOTHING;
...
-- ==============================================
INSERT INTO jobs (
  id, tenant_id, user_id, title, department, location, type, status,
...
INSERT INTO channels (
  id, tenant_id, user_id, name, type, status, applicants_count,
...
-- ==============================================
INSERT INTO resumes (
  id, tenant_id, user_id, candidate_name, email, phone, position, status,
...
INSERT INTO ai_match_results (
  id, tenant_id, user_id, resume_id, job_id, is_match, match_score, reason, strengths, weaknesses, recommendation, analyzed_at
)
VALUES 
  (
...
INSERT INTO recruitment_tasks (
  id, tenant_id, user_id, job_id, job_title, status, created_by,
...
INSERT INTO interviews (
  id, tenant_id, user_id, candidate_id, candidate_name, position,
...
INSERT INTO chat_messages (
  id, tenant_id, user_id, session_id, role, content, message_type, created_at
)
VALUES 
  -- Session 1: 招聘咨询 - 前端工程师
  ('71000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000001', 'assistant', '你好！我是AI招聘助手，很高兴为您服务。我可以帮助您筛选简历、安排面试、分析招聘数据等。请问需要什么帮助？', 'text', '2025-01-13 09:00:00+00'),
  ('71000000-0000-0000-0000-000000000002', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000001', 'user', '帮我筛选一下前端工程师的简历', 'text', '2025-01-13 09:01:00+00'),
  ('71000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000001', 'assistant', '我已经为你筛选了最近的简历。发现3份高质量简历匹配高级前端工程师职位，其中张伟的简历匹配度最高（92分），有6年经验和字节跳动背景。李明的简历也不错（85分），有阿里巴巴经验。建议优先面试这两位候选人。', 'text', '2025-01-13 09:02:00+00'),
  ('71000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000001', 'user', '张伟的技术栈怎么样？', 'text', '2025-01-13 15:25:00+00'),
  ('71000000-0000-0000-0000-000000000005', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000001', 'assistant', '张伟的技术栈非常匹配我们的需求：\n\n✅ 前端框架：React、Vue.js\n✅ 开发语言：TypeScript、JavaScript\n✅ 构建工具：Webpack、Vite\n✅ 后端技能：Node.js、Express\n✅ 数据库：MySQL、MongoDB\n\n他在字节跳动负责抖音Web端核心功能开发，有大型项目架构经验。建议安排技术面试。', 'text', '2025-01-13 15:30:00+00'),
  
  -- Session 2: 面试安排 - HR经理
  ('71000000-0000-0000-0000-000000000006', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000002', 'user', '帮我安排李娜的面试', 'text', '2025-01-12 14:00:00+00'),
  ('71000000-0000-0000-0000-000000000007', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000002', 'assistant', '好的，我来帮您安排李娜（HR经理职位）的面试。根据她的简历，她有4年HR经验，目前在阿里巴巴担任招聘经理。建议安排以下面试流程：\n\n1. 初面：HR部门负责人（30分钟）\n2. 专业面试：业务部门经理（45分钟）\n3. 终面：VP或总监级别（30分钟）\n\n您希望什么时候开始安排？', 'text', '2025-01-12 14:01:00+00'),
  ('71000000-0000-0000-0000-000000000008', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000002', 'user', '明天下午可以吗？', 'text', '2025-01-12 16:40:00+00'),
  ('71000000-0000-0000-0000-000000000009', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000002', 'assistant', '明天下午的时间安排如下：\n\n📅 1月13日（明天）\n🕐 14:00-14:30 初面（HR王总监）\n🕐 14:45-15:30 专业面试（业务部门张经理）\n🕐 15:45-16:15 终面（VP李总）\n\n面试地点：公司会议室A\n已发送面试邀请邮件给候选人，请确认是否需要调整。', 'text', '2025-01-12 16:45:00+00'),
  
  -- Session 3: 数据分析 - 招聘效果
  ('71000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000003', 'user', '分析一下这个月的招聘数据', 'text', '2025-01-11 10:00:00+00'),
  ('71000000-0000-0000-0000-000000000011', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000003', 'assistant', '📊 本月招聘数据分析（截至1月11日）：\n\n📈 **整体数据**\n• 收到简历：156份（↑23% vs上月）\n• 安排面试：45场（↑15% vs上月）\n• 发出offer：12个（↑8% vs上月）\n• 入职确认：8人（↑33% vs上月）\n\n🎯 **转化率**\n• 简历→面试：28.8%（行业平均25%）\n• 面试→offer：26.7%（行业平均20%）\n• offer→入职：66.7%（行业平均70%）\n\n💡 **建议**：offer接受率略低，建议优化薪酬包装和候选人体验。', 'text', '2025-01-11 10:05:00+00'),
  ('71000000-0000-0000-0000-000000000012', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000003', 'user', '哪个渠道效果最好？', 'text', '2025-01-11 11:25:00+00'),
  ('71000000-0000-0000-0000-000000000013', '00000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', '70000000-0000-0000-0000-000000000003', 'assistant', '🏆 **招聘渠道效果排名**：\n\n1. **BOSS直聘** - 42份简历，转化率32%\n2. **智联招聘** - 38份简历，转化率28%\n3. **猎聘网** - 31份简历，转化率25%\n4. **拉勾网** - 28份简历，转化率22%\n5. **内推** - 17份简历，转化率65%（质量最高）\n\n💰 **成本效益**：内推成本最低且质量最高，建议加大内推激励力度。BOSS直聘虽然量大但成本适中，可以作为主要渠道。', 'text', '2025-01-11 11:30:00+00')
ON CONFLICT (id) DO NOTHING;
